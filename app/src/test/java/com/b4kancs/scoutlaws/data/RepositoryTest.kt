...
class RepositoryTest {
    ...
    fun repositoryShouldLoadScoutLawsInOrder() {
        ...
        val pickChooseScoutLaw1 = PickerScoutLaw(scoutLaw1, sl1PickText, sl1PickOptions.toList())
        val pickChooseScoutLaw2 = PickerScoutLaw(scoutLaw2, sl2PickText, sl2PickOptions.toList())

        val repository = Repository(stubResources, StubUserDataStore(), mockSharedPreferences)

        assertEquals(scoutLaw1, repository.scoutLaws[0])
        assertEquals(scoutLaw2, repository.scoutLaws[1])
        assertEquals(pickChooseScoutLaw1, repository.pickerScoutLaws[0])
        assertEquals(pickChooseScoutLaw2, repository.pickerScoutLaws[1])
    }
    ...
    fun settingUserDataShouldSetCorrectValues() {
        ...
        repository.apply {
            ...
            assertEquals(20L, getBestPickerTime())
